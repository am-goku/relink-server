import { clearNotifications, fetchNotifications, readNotification } from "../helpers/notificationHelper";
...
export const setReadStatus = (req, res) => {
    try {
        ...
        readNotification(notifyId).then((response) => {
...
export const deleteNotification = (req, res) => {
    try {
        const userId = req.params.userId;
        clearNotifications(userId).then((response) => {
            res.status(200).json(response);
        }).catch((error) => {
            res.status(500).json(error)
        })
